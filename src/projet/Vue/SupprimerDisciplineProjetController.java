 ...
 * @author ameliefiems
 */
public class SupprimerDisciplineProjetController implements Initializable, ControlledEcran {
    ...
    private ObservableList<Discipline> listDis = FXCollections.observableArrayList();
    private List<Discipline> ld = new ArrayList();
    @FXML
    private TextField titre;
    @FXML
    private ListView<Discipline> disListView;
    Discipline dis;
    String ti;
    ...
    public void rech() {
        ...
        ld = Principal.pm.listeDisciplineProjet(titre.getText(), pg);
        if (ld != null) {
            ld.forEach((d) -> {
                listDis.add(d);
            });
        }
        disListView.setItems(listDis);

        disListView.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                dis = disListView.getSelectionModel().getSelectedItem();
    ...
    public void supprimer() {
        ...
        if (!titre.getText().equals("") && dis != null) {
            titre.setText(ti);
            Temps t = Principal.pm.getTemps(dis, Principal.pm.getProjet(p, titre.getText()));
            if (t != null) {
                Principal.pm.supprimer(t);
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Information modification de la suppresion de la discipline");
                alert.setHeaderText(null);
                alert.setContentText("Vous avez bien supprimé la discipline");
                alert.showAndWait();
            } else {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Information modification de la suppresion de la discipline");
